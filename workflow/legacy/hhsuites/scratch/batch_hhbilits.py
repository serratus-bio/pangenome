...
if 1:
    ...
    all_segs=[Path(i) for i in glob('_data/genome_fasta/*segs.fasta')]
    # print(len(all_segs))
    ...
    qbar=tqdm.tqdm(total=len(used_genome))
    def callback(i):
        qbar.update()
    ...
    for infile in used_genome:
        r=pool.apply_async(tmp,(infile,),callback=callback)
        res.append(r)
    pool.close()
    pool.join() 
    ...
    pkl.dump(_,open('blit_out_all_other.pkl','wb'))
